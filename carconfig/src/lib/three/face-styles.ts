...
const FACES: Record<FaceFamily, FaceDesign> = {
  ...
  porsche_911: {
    // High on the face, where the wings meet the nose, as on the real car.
    headlights: pair(F(0.7, 0.55, 0.15, 0.4, 2.1)),
    drl: [0.07, -0.07].flatMap((dx) =>
      [0.16, -0.16].flatMap((dy) => pair(F(0.7 + dx, 0.55 + dy, 0.018, 0.05, 6, { lift: 0.005 }))),
    ),
    projectors: pair(F(0.7, 0.55, 0.055, 0.13, 2, { lift: 0.005 })),
    surround: [],
    grille: [],
